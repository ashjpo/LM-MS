 ...
 *
 */
public class html_cache{
	//临时
	String cache_root_path="D:/CacheHtmlTest";
	//超出这个时间缓存将被删除
	long cache_limit_time=60;	//s
	public html_cache(Main_Context mc) {
		cache_root_path=mc.config_obj.html_cache_dir_path;
		cache_limit_time=mc.config_obj.html_cache_limit_time;
	...
	public String get_cache(match_ask mAsk) {
        ...
        for (int i = 0; i < filelist.length; i++) {
        	String file_name=filelist[i].replaceAll(".html", "");
	...
	public boolean set_cache(match_ask mAsk,String message,Boolean force) {
        ...
        for (int i = 0; i < filelist.length; i++) {
        	...
        	if(force) {	//全部更新
        		...
        		File last_file = new File(cache_root_path + "/" + filelist[i]);
        		last_file.delete();
        		String new_file_name=service_name+"_"+version+"_"+ask_name+"_"+current_time+".html";
